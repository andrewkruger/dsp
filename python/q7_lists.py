...
def front_x(words):
    ...
    ['xanadu', 'xyz', 'aardvark', 'apple', 'mix']
    """
    # Following has separation complexity O(n)
    xwords = []
    swords = []
    for word in words:
      if word[0] == 'x':
    ...
    #This could also work, but would take longer:
    '''return sorted([x for x in words if x[0]=='x']) +\
        sorted([x for x in words if x[0]!='x'])'''
